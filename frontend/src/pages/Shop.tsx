    ...
    <CyberCard onClick={() => navigate(`/product/${product.id}`)} className="group overflow-hidden flex flex-col h-full cursor-pointer hover:-translate-y-[4px] hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(139,92,246,0.12)] bg-cyber-card border border-cyber-border hover:border-cyber-purple/50">
      <div className="aspect-[4/5] bg-gradient-to-br from-cyber-surface to-cyber-card flex items-center justify-center relative overflow-hidden">
        ...
        {product.images && product.images.length > 0 ? (
          ...
          <ShoppingBag className="w-14 h-14 text-cyber-purple/20 group-hover:text-cyber-purple/50 transition-transform duration-500 ease-out group-hover:scale-[1.05]" />
        )}

        <div className="absolute top-4 right-4 font-mono text-[10px] text-cyber-purple/30 z-10 pointer-events-none">
      ...
      <div className="p-4 flex flex-col justify-between flex-grow">
        <div>
          ...
          <div className="flex gap-1.5 mb-3">
            {product.colors.map((color, i) => (
              <div
                key={i}
                className="w-3.5 h-3.5 rounded-full border border-cyber-border"
        ...
        <div className="pt-2">
          <span className="font-mono text-gray-900 dark:text-white text-lg font-semibold drop-shadow-[0_0_6px_rgba(139,92,246,0.4)]">
